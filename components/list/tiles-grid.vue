...
export default {
  ...
  methods: {
    ...
    generateListClass(index) {
      ...
      let className =
        "tile" + (this.thumbnailMode ? " tile--thumbnail-mode" : "");
      if (len % this.numColumns == 0 && index == len - this.numColumns) {
        className += " last-2nd";
      } else if (
        this.numColumns == 2 &&
        len % this.numColumns == 1 &&
        index == len - 1
      ) {
    ...
    adjustSize() {
      ...
      let maxH = [].slice.call(tiles).reduce(
        (max, tile) => {
