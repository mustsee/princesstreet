<template>
  <div class="list-box">
    <div v-for="(item, index) in list" :class="getClassName(item)" :key="index">
      ...
      <lazy-image v-if="item.img" :src="item.img" />
      <h3 v-if="item.name" class="list-box__item-name">{{ item.name }}</h3>
      <h3
        v-if="item.subnamehead"
        class="list-box__item-subname"
        v-html="item.subnamehead"
      ></h3>
      <div
        v-if="item.subname"
        class="list-box__item-subname"
      ...
      <div v-if="item.table">
        <Table :headers="item.tableHeaders" :items="item.table" />
      </div>
      <div v-if="item.note" class="list-box__item-desc">
        <b>Note:</b> {{ item.note }}
      </div>
...
export default {
  ...
  methods: {
    getClassName(item) {
      ...
      if (item.customListWithNumber !== undefined)
        className = "list-box__item custom-list";
